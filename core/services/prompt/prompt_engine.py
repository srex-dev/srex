...
class ResponseProcessor:
    ...
    def _extract_json_block(self, text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # logger.debug(f"🔍 Extracted JSON block for {self.step_name}, length: {len(json_str)}")
            # logger.debug(f"🔍 JSON block start: {json_str[:100]}...")
            # logger.debug(f"🔍 JSON block end: ...{json_str[-100:]}")
            
            # Try to parse the extracted JSON directly first
            try:
                # logger.debug(f"🔍 Trying to parse extracted JSON for {self.step_name}")
                result = json.loads(json_str)
                # logger.debug(f"✅ Extracted JSON parse successful for {self.step_name}")
                return result
            except Exception as e:
                # logger.debug(f"❌ Extracted JSON parse failed for {self.step_name}: {e}")
                pass  # Fall through to cleaning
            
            # Check if JSON is missing closing brace (common LLM issue)
            if json_str.strip().endswith(']') and not json_str.strip().endswith('}]'):
            ...
            # Check if JSON is missing closing bracket (common LLM issue)
            ...
            # logger.debug(f"🔍 Starting cleaning process for {self.step_name}")
            # logger.debug(f"🔍 Original JSON before cleaning: {json_str[:200]}...")
            ...
            # logger.debug(f"🔍 After basic cleaning: {json_str[:200]}...")
            ...
            # logger.debug(f"🔍 After quote fixing: {json_str[:200]}...")
            ...
            # logger.debug(f"🔍 Final cleaned JSON for {self.step_name}: {json_str[:200]}...")
